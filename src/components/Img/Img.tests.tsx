...
describe('Img Component', () => {
    test('renders image and is visible', () => {
        render(<Img src="https://placehold.co/600x400?text=Hello+World" alt="Test" />);
    ...
    test('changes appearance when disabled', () => {
        const { rerender } = render(<Img src="https://placehold.co/600x400?text=Hello+World" disabled={false} />);
        const containerElement = screen.getByTestId('img-container');
        const enabledStyle = window.getComputedStyle(containerElement);

        rerender(<Img src="https://placehold.co/600x400?text=Hello+World" disabled={true} />);
